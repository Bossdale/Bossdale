...
###  PocketClass
**AI Model Engineer & Mobile Developer**
- Built an **offline-first learning app** using React Native (Expo + TypeScript)
...
- Developed a **smart medication system** with React + ESP32 IoT
...
- Built a **job/service platform** using React + Spring Boot  
...
###  Languages
`Java` `C++` `TypeScript` `Python` `C#` `GDScript` `C`

###  Frameworks & Libraries
`React` `React Native` `Tailwind CSS` `Spring Boot` `Django` `TensorFlow`

###  Databases
`MySQL` `Firebase`

###  Tools & Platforms
`Git` `GitHub` `VS Code` `IntelliJ IDEA` `PyCharm` `Xcode` `Godot` `Figma` `Canva`
